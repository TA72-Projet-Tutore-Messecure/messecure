...
export const CAsideConversation: FC<CAsideConversationProps> = ({
  user,
  active,
  onClick,
}) => {
    ...
    >
      ...
      <div className="flex flex-col justify-between items-start w-full max-w-[17vw]">
        <div className="w-full flex flex-row items-center justify-between">
          <span
            className={`text-sm font-bold truncate ${active ? "text-white" : "dark:text-white"}`}
          >
            Madoc Lefèvre
          </span>
          <span
            className={`text-xs ${active ? "text-white" : "dark:text-white/30"}`}
          >
            12:00
          </span>
        </div>
        <span
          className={`truncate w-full overflow-hidden text-ellipsis ${active ? "text-white" : "text-default-500 dark:text-default-500"}`}
        >
